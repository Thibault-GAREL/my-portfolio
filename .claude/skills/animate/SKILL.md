---
name: animate
description: Build an animation from scratch in this Next.js + Tailwind portfolio, making the decisions in the order that determines whether it feels right — should it animate at all, what purpose, which tool, which properties, which curve and duration, how it interrupts, how it exits. Writes the implementation. Use when asked to animate something, add motion, make a component feel alive, or build a transition in this web project. Do NOT use for GIF/MP4 generated by Python code (see the global schema-animation skill) nor for Remotion video (see thibault-remotion).
---
...
Scope note for this repo: this is the web UI counterpart only. Procedural GIF/MP4 built with Pillow or OpenCV belongs to the global `schema-animation` skill, and React video belongs to `thibault-remotion` plus the official `remotion-*` skills. The companion skills from the upstream repo (`review-animations`, `improve-animations`, `find-animation-opportunities`, `animate-expo`) are deliberately not installed here — where the text below points at them, apply the rule yourself rather than trying to invoke them.
